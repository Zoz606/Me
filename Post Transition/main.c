...
{ //enter the number of towns
    ...
    for (int i = 0; i < towns_count; i++)
    {
        ...
        printf("Please enter the name of the town in uppercse : ");
        scanf("%s", towns[i].name);

        printf("Please enter the number of offices in this town : ");
        scanf("%d", &towns[i].offices_count);

        towns[i].offices = malloc(sizeof(post_office) * towns[i].offices_count);

        //offices => max weight , min weight ,package_count
        for (int j = 0; j < towns[i].offices_count; j++)
        {
            printf("Package count :");
            scanf("%d", &towns[i].offices[j].packages_count);
            printf("min weight: ");
            scanf("%d", &towns[i].offices[j].min_weight);
            printf("max weight: ");
            scanf("%d", &towns[i].offices[j].max_weight);

            towns[i].offices[j].packages = malloc(sizeof(package) * towns[i].offices[j].packages_count);
            // data of packages => id , weight
            for (int k = 0; k < towns[i].offices[j].packages_count; k++)
            {
                towns[i].offices[j].packages[k].id = malloc(sizeof(char) * MAX_STRING_LENGTH);

                printf("Enter the packace id in lowercase: ");
